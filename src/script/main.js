class CountDown {
    ...
    complete() {
        if (typeof this.onComplete === 'function') onComplete();
    }
    getTime() {
        return {
    ...
    update() {
        if (typeof this.onRender === 'function') this.onRender(this.getTime());
    }
...
const year = document.querySelector('.year').innerHTML = getNewYear().getFullYear();
